  ...
  return (
    <div className={`flex items-end justify-center sm:justify-start space-x-7 bg-gradient-to-b to-black ${color}`}>
        ...
        <div className="place-self-end text-center sm:text-left">
            ...
            <a rel="noreferrer" href={playlist?.owner?.external_urls?.spotify} target="_blank" className="font-bold text-sm cursor-pointer hover:underline">{playlist?.owner?.id}</a>
        </div>
        </div>
    </div>
  )
